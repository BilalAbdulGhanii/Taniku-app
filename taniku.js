...
/* ===== TOAST & MODAL ===== */
function toast(pesan,tipe){tipe=tipe||'info';const w=document.getElementById('toast-wrap');if(!w)return;const i={success:'check-circle-2',error:'alert-circle',info:'info'}[tipe]||'info';const el=document.createElement('div');el.className='toast '+tipe;el.innerHTML='<i data-lucide="'+i+'"></i><div style="flex:1">'+esc(pesan)+'</div>';w.appendChild(el);if(window.lucide)lucide.createIcons();setTimeout(()=>{el.style.opacity='0';el.style.transition='opacity .3s';setTimeout(()=>el.remove(),300)},3200)}
...
function tutupModal(){const r=document.getElementById('modal-root');if(r)r.innerHTML=''}
...
/* ===== ROUTER ===== */
function rute(){const hash=location.hash||'#/masuk';HALAMAN_KINI=hash;const b=hash.replace('#/','').split('/').filter(Boolean);
  if(b[0]==='admin'||b[0]==='pembeli'){if(!SESI){location.hash='#/masuk';return}if(b[0]==='admin'&&SESI.peran!=='admin'){location.hash='#/pembeli';return}if(b[0]==='pembeli'&&SESI.peran!=='pembeli'){location.hash='#/admin';return}}
  else if(b[0]==='masuk'||b[0]==='daftar'||b[0]===''){if(SESI){location.hash=SESI.peran==='admin'?'#/admin':'#/pembeli';return}}
  render()}
function render(){const app=document.getElementById('app');if(!app)return;const hash=HALAMAN_KINI||'#/masuk';const b=hash.replace('#/','').split('/').filter(Boolean);
  if(!b.length||b[0]==='masuk')app.innerHTML=halamanMasuk();
  else if(b[0]==='daftar')app.innerHTML=halamanDaftar();
  else if(b[0]==='admin')app.innerHTML=layout(SESI,MENU_ADMIN,halamanAdmin(b.slice(1)));
  else if(b[0]==='pembeli')app.innerHTML=layout(SESI,MENU_PEMBELI,halamanPembeli(b.slice(1)));
  else app.innerHTML=halamanMasuk();
  if(window.lucide)lucide.createIcons();
  if(window.__CHART_AFTER){const f=window.__CHART_AFTER;window.__CHART_AFTER=null;setTimeout(()=>{if(CHART_AKTIF){try{CHART_AKTIF.destroy()}catch(e){}}CHART_AKTIF=null;try{f()}catch(e){console.error(e)}},80)}}
...
function adminCuaca(){const s=SENSOR;
  window.__CHART_AFTER=()=>{const c=document.getElementById('chartCuaca');if(!c||!window.Chart)return;
    CHART_AKTIF=new Chart(c,{type:'line',data:{labels:s.tren24Jam.map(t=>t.jam),
      datasets:[{label:'Suhu (°C)',data:s.tren24Jam.map(t=>t.suhu),borderColor:'#5B9BD5',backgroundColor:'rgba(91,155,213,.15)',tension:.4,fill:true},
  ...
  return '<div class="page-head"><div><h1 class="page-title">Cuaca & IoT</h1><p class="page-sub">'+esc(s.lokasi)+' • '+esc(s.node)+'</p></div>'+
    '<span class="badge badge-green"><i data-lucide="radio"></i> Sensor Aktif</span></div>'+
    '<div class="two-col mb-5"><div class="weather-hero">'+
    '<div class="text-xs uppercase tracking-wide" style="opacity:.85">'+esc(s.lokasi)+'</div>'+
    '<div class="weather-temp">'+s.suhu+'°C</div><div class="text-sm" style="opacity:.9">'+esc(s.kondisi)+' • Terasa '+s.terasa+'°C</div>'+
    '<div class="weather-row">'+wc('Kel. Udara',s.kelembapanUdara+'%')+wc('Kel. Tanah',s.kelembapanTanah+'%')+wc('Curah Hujan',s.curahHujan+' mm/j')+wc('Angin',s.kecepatanAngin+' km/j')+wc('Cool Box',s.suhuCoolBox+'°C')+wc('Arah',s.arahAngin)+'</div>'+
    '</div>'+
    '<div class="card"><div class="card-head"><div class="card-title">Rekomendasi</div></div><div class="card-body">'+
    '<div class="flex items-start gap-3 mb-3"><span class="badge badge-yellow">⚠️</span><div><div class="font-bold text-sm">Kelembapan Tinggi</div><div class="text-xs text-muted mt-1">Pantau penyakit jamur pada cabai dan tomat.</div></div></div>'+
    '<div class="flex items-start gap-3"><span class="badge badge-green">💧</span><div><div class="font-bold text-sm">Tanah Ideal</div><div class="text-xs text-muted mt-1">Tunda penyiraman hingga sore.</div></div></div>'+
    '</div></div></div>'+
    '<div class="card mb-5"><div class="card-head"><div><div class="card-title">Tren 24 Jam</div></div></div><div class="card-body"><div class="chart-wrap"><canvas id="chartCuaca"></canvas></div></div></div>'+
    '<div class="card"><div class="card-head"><div><div class="card-title">Prakiraan 4 Hari</div></div></div><div class="card-body"><div class="product-grid">'+
    s.prakiraan.map(p=>'<div class="card" style="box-shadow:none"><div class="card-body text-center"><div class="font-bold mb-2">'+p.hari+'</div>'+
    '<div style="font-size:36px;margin:8px 0">'+(p.ikon==='rain'?'🌧️':p.ikon==='sun'?'⛅':'☁️')+'</div>'+
    '<div class="font-extrabold text-g">'+p.suhu+'</div><div class="text-xs text-muted mt-1">'+esc(p.kondisi)+'</div>'+
    '<div class="text-xs text-muted mt-1">Hujan '+p.hujan+'% • Lembap '+p.lembap+'%</div></div></div>').join('')+
    '</div></div></div>'}
function wc(l,n){return '<div class="weather-cell"><span style="opacity:.85">'+l+'</span><b>'+n+'</b></div>'}

function adminNotif(){const list=DB.notifikasi.filter(n=>n.peran==='admin').sort((a,b)=>new Date(b.waktu)-new Date(a.waktu));
...
function notifItem(n){const ik={pesanan:'shopping-bag',produk:'package',komplain:'message-square',cuaca:'cloud-sun',persediaan:'boxes',mitra:'users'};
  ...
  return '<div class="notif-item'+(n.dibaca?'':' unread')+'" data-aksi="baca-notif" data-id="'+n.id+'">'+
    ...
    '<div class="notif-body"><div class="notif-title">'+esc(n.judul)+'</div>'+
    '<div class="notif-desc">'+esc(n.pesan)+'</div>'+
    '<div class="notif-time">'+relatif(n.waktu)+'</div></div></div>'}
...
function pembeliCheckout(){
  ...
  return '<a href="#/pembeli/keranjang" class="btn btn-ghost btn-sm mb-4"><i data-lucide="arrow-left"></i> Kembali</a>'+
    ...
    '<i data-lucide="check-circle"></i> Bayar Sekarang</button>'+
...
function pembeliCuaca(){const s=SENSOR;
  window.__CHART_AFTER=()=>{const c=document.getElementById('chartCuacaPembeli');if(!c||!window.Chart)return;
    CHART_AKTIF=new Chart(c,{type:'line',data:{labels:s.tren24Jam.map(t=>t.jam),
      datasets:[{label:'Suhu (°C)',data:s.tren24Jam.map(t=>t.suhu),borderColor:'#5B9BD5',backgroundColor:'rgba(91,155,213,.15)',tension:.4,fill:true},
      {label:'Kel. Tanah (%)',data:s.tren24Jam.map(t=>t.kelembapanTanah),borderColor:'#2F6B4F',backgroundColor:'rgba(47,107,79,.12)',tension:.4,fill:true}]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}},scales:{y:{beginAtZero:false},x:{grid:{display:false}}}}})};
  return '<div class="page-head"><div><h1 class="page-title">Cuaca & IoT Kebun</h1><p class="page-sub">Data real-time dari sensor Taniku</p></div>'+
    '<span class="badge badge-green"><i data-lucide="radio"></i> Sensor Aktif</span></div>'+
    '<div class="weather-hero mb-5"><div class="text-xs uppercase tracking-wide" style="opacity:.85">'+esc(s.lokasi)+'</div>'+
    '<div class="weather-temp">'+s.suhu+'°C</div><div class="text-sm" style="opacity:.9">'+esc(s.kondisi)+' • Terasa '+s.terasa+'°C</div>'+
    '<div class="weather-row">'+wc('Kel. Udara',s.kelembapanUdara+'%')+wc('Kel. Tanah',s.kelembapanTanah+'%')+wc('Curah Hujan',s.curahHujan+' mm/j')+'</div></div>'+
    '<div class="card mb-5"><div class="card-head"><div><div class="card-title">Tren 24 Jam</div></div></div><div class="card-body"><div class="chart-wrap"><canvas id="chartCuacaPembeli"></canvas></div></div></div>'+
    '<div class="card"><div class="card-head"><div><div class="card-title">Prakiraan 4 Hari</div></div></div><div class="card-body"><div class="product-grid">'+
    s.prakiraan.map(p=>'<div class="card" style="box-shadow:none"><div class="card-body text-center"><div class="font-bold mb-2">'+p.hari+'</div>'+
    '<div style="font-size:36px;margin:8px 0">'+(p.ikon==='rain'?'🌧️':p.ikon==='sun'?'⛅':'☁️')+'</div>'+
    '<div class="font-extrabold text-g">'+p.suhu+'</div><div class="text-xs text-muted mt-1">'+esc(p.kondisi)+'</div></div></div>').join('')+
    '</div></div></div>'}
...
/* ===== AKSI ===== */
function aksiMasuk(f){const d=new FormData(f);const email=(d.get('email')||'').trim().toLowerCase();const sandi=d.get('sandi');
  const u=DB.pengguna.find(x=>x.email.toLowerCase()===email&&x.sandi===sandi);
  if(!u){toast('Email atau kata sandi salah.','error');return}
  SESI={id:u.id,nama:u.nama,email:u.email,peran:u.peran,telepon:u.telepon,alamat:u.alamat,avatar:u.avatar,foto:u.foto};
  simpanSesi(SESI);toast('Selamat datang, '+u.nama+'!','success');
  location.hash=u.peran==='admin'?'#/admin':'#/pembeli'}
function aksiDaftar(f){const d=new FormData(f);const nama=(d.get('nama')||'').trim();const email=(d.get('email')||'').trim().toLowerCase();
  const sandi=d.get('sandi');const peran=d.get('peran')||'pembeli';
  if(!nama||!email||!sandi){toast('Semua kolom wajib diisi.','error');return}
  if(DB.pengguna.some(u=>u.email.toLowerCase()===email)){toast('Email sudah terdaftar.','error');return}
  const u={id:uid('u'),nama,email,sandi,peran,telepon:'',alamat:'',avatar:inisial(nama)};
  DB.pengguna.push(u);simpanDB();
  SESI={id:u.id,nama:u.nama,email:u.email,peran:u.peran,telepon:'',alamat:'',avatar:u.avatar};
  simpanSesi(SESI);toast('Akun berhasil dibuat.','success');
  location.hash=peran==='admin'?'#/admin':'#/pembeli'}
function aksiKeluar(){SESI=null;simpanSesi(null);location.hash='#/masuk';toast('Anda telah keluar.','info')}
function aksiTambahKeranjang(id){const p=DB.produk.find(x=>x.id===id);if(!p)return;
  ...
  DB.keranjang[SESI.id][id]=sek+1;simpanDB();
  toast(p.nama+' ditambahkan ke keranjang.','success');render()}
function aksiHapusItem(id){if(!DB.keranjang[SESI.id])return;delete DB.keranjang[SESI.id][id];simpanDB();toast('Produk dihapus.','info');render()}
function aksiUbahQty(id,d){if(!DB.keranjang[SESI.id])return;const p=DB.produk.find(x=>x.id===id);if(!p)return;
  let q=(DB.keranjang[SESI.id][id]||1)+d;if(q<1)q=1;if(q>p.stok){toast('Stok tidak cukup.','error');return}
  DB.keranjang[SESI.id][id]=q;simpanDB();render()}
function aksiHapusProduk(id){const p=DB.produk.find(x=>x.id===id);if(!p)return;
  if(!confirm('Hapus produk "'+p.nama+'"?'))return;
  DB.produk=DB.produk.filter(x=>x.id!==id);simpanDB();toast('Produk dihapus.','success');render()}
function aksiBacaNotif(id){const n=DB.notifikasi.find(x=>x.id===id);if(!n)return;
  n.dibaca=true;simpanDB();
  if(n.tautan&&n.tautan!==HALAMAN_KINI){location.hash=n.tautan.replace('#','')}else{render()}}
  ...
  document.addEventListener('click',e=>{
    ...
    if(mc&&!e.target.closest('[data-aksi]')){
      ...
      if(mc.dataset.kirim){
        ...
        const btnBayar=document.querySelector('[data-aksi="proses-bayar"]');
    ...
    switch(a){
      ...
      case 'simpan-produk-baru':{
        ...
        simpanDB();tutupModal();toast('Produk ditambahkan.','success');render();break;
      }
      ...
      case 'terapkan-promo':{
        ...
        const subtotal=Number(document.querySelector('[data-aksi="proses-bayar"]').dataset.subtotal);
